...
def main():
    ...
    with st.sidebar:
        ...
        uploaded_files = st.file_uploader(
            "Choose file(s)", 
            type=['pdf', 'pptx', 'docx', 'xlsx', 'jpg', 'png', 'html', 'csv', 'json', 'xml'], # Will add 'mp3', 'wav' back in once PR merged for MarkItDown
    ...
    if uploaded_files:
        # Process all files first, storing results in a dictionary
        extracted_texts = {}
        ...
        for uploaded_file in uploaded_files:
            with st.spinner(f"Processing {uploaded_file.name}..."):
                try:
                    ...
                    extracted_texts[uploaded_file.name] = text_content
                except Exception as e:
                    st.error(f"Error processing document {uploaded_file.name}: {str(e)}")
                    extracted_texts[uploaded_file.name] = f"[Error: {str(e)}]"

        # --- RIGHT-ALIGNED DOWNLOAD ALL SECTION ---
        ...
        # Create two columns so heading is on the left, button is on the right
        ...
        for index, uploaded_file in enumerate(uploaded_files):
            ...
            with tabs[index]:
                # Another columns layout for right-aligning the individual download
